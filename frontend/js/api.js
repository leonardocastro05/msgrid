// ── Gestión del token ──────────────────────────────────────────────
function getToken()         { return localStorage.getItem('srs_token'); }
function setToken(t)        { localStorage.setItem('srs_token', t); }
function removeToken()      { localStorage.removeItem('srs_token'); localStorage.removeItem('srs_admin'); }
function getAdmin()         { const a = localStorage.getItem('srs_admin'); return a ? JSON.parse(a) : null; }
function setAdmin(a)        { localStorage.setItem('srs_admin', JSON.stringify(a)); }
...
async function fetchResultado(liga, temporada, carrera, sesion = 'carrera') {
  return apiFetch(`/resultado?liga=${liga}&temporada=${temporada}&carrera=${carrera}&sesion=${sesion}`);
}
...
async function actualizarPartido(torneoId, datos) {
  return apiFetch(`/torneos/${torneoId}/partido`, { method: 'PUT', body: JSON.stringify(datos) });
}